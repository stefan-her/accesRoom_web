...
CREATE TRIGGER `dispatcher` AFTER INSERT ON `##TB1##` FOR EACH ROW 
	...
	THEN 
		CASE   
			...
			WHEN (JSON_UNQUOTE(new.content->'$.tb')) = "##TB2##"
		    	THEN
	    	...
	    	WHEN (JSON_UNQUOTE(new.content->'$.tb')) = "##TB3##"
	    	THEN
				SET @fields = JSON_EXTRACT(new.content,'$.fields');
	            SET @fieldsName = JSON_KEYS(@fields);
	            SET @firstname = null;
	        ...
	        WHEN (JSON_UNQUOTE(new.content->'$.tb')) = "##TB5##"
	    	THEN
				SET @fields = JSON_EXTRACT(new.content,'$.fields');
	            SET @fieldsName = JSON_KEYS(@fields);
	    	...
	    	WHEN (JSON_UNQUOTE(new.content->'$.tb')) = "##TB4##"
	    	THEN
				SET @fields = JSON_EXTRACT(new.content,'$.fields');
	            SET @fieldsName = JSON_KEYS(@fields);
	    	...
	    	WHEN (JSON_UNQUOTE(new.content->'$.tb')) = "##TB6##"
	    	THEN
				SET @fields = JSON_EXTRACT(new.content,'$.fields');
	            SET @fieldsName = JSON_KEYS(@fields);
	            SET @name = NULL;
...
CREATE TRIGGER `verifApiKey` BEFORE INSERT ON `##TB1##` FOR EACH ROW IF (
			SELECT @ID := `id_##TB3##`
			FROM `##TB3##`
