...
if(empty($res->values["error"]) || !$res->values["error"]) {
    ...
    $path = realpath($temp_dir . DIRECTORY_SEPARATOR . Globals::INSERTSTARTLOGS_FILE);
    
    if(file_exists($temp_dir . DIRECTORY_SEPARATOR . Globals::INSERTSTARTLOGS_INSERTED)) {
        $v = file_get_contents($temp_dir . DIRECTORY_SEPARATOR . Globals::INSERTSTARTLOGS_INSERTED);
    ...
    if(count($content) > ($line)) {
        ...
        if(isset($content[$line])) {
            ...
            if(isset($res->values["done"])) {
                $a["status"] = "ok";
                file_put_contents($temp_dir . DIRECTORY_SEPARATOR . Globals::INSERTSTARTLOGS_INSERTED, $line);
            }
            else { $a["status"] = "no"; }
